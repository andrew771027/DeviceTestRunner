...
@pytest.mark.parametrize(argnames="test_case_id, stage, step_name, stdout, stderr", 
                         argvalues=[
                            (
                                "test_case_001",
                                "test_stage", 
...
def test_save_step_output(tmp_path: Path, test_case_id:str, stage:str, step_name:str, stdout:str, stderr:str):
    ...
    stdout_path = artifact_manager.save_step_stdout(run_dir=run_dir, stage=stage, step_name=step_name, stdout=stdout)

    stderr_path = artifact_manager.save_step_stderr(run_dir=run_dir, stage=stage, step_name=step_name, stderr=stderr)
